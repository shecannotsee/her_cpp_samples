...
void main() {
  ...
  res = PQexec(conn,"SELECT pg_catalog.set_config('search_path', 'private', false)");/* check set */ {
  ...
  res = PQexec(conn, "DECLARE myportal CURSOR FOR select * from private.book");/* check success */ {
    if (PQresultStatus(res) != PGRES_COMMAND_OK) {
