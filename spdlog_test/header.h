...
void main() {
  ...
  /* set [Time interval] to write files from buffer*/ {
    ...
    spdlog::flush_every(std::chrono::milliseconds(1));// 毫秒,千分之一秒
    spdlog::flush_every(std::chrono::microseconds(1));// 微秒,百万分之一秒
    spdlog::flush_every(std::chrono::nanoseconds(1));// 纳秒,十亿分之一秒
